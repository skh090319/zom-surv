...
function drawMonsterGuide() {
  ...
  bossEntries.forEach((entry,i)=>{const x=bossX,y=bossStart+i*(bossH+gap)-guideScrollY;if(y+bossH<top||y>canvas.height)return;
    ...
    const fogTime=performance.now()*.00032;ctx.save();ctx.globalCompositeOperation="screen";for(let mist=0;mist<5;mist++){const drift=Math.sin(fogTime*(1+mist*.08)+mist*1.7)*artW*.09,fx=cx+(mist-2)*artW*.16+drift,fy=cy+66+Math.cos(fogTime*.8+mist)*13,fr=54+mist*7;const fog=ctx.createRadialGradient(fx,fy,3,fx,fy,fr*1.55);fog.addColorStop(0,`${entry.color}${mist%2?"38":"2c"}`);fog.addColorStop(.48,`${entry.color}18`);fog.addColorStop(1,`${entry.color}00`);ctx.fillStyle=fog;ctx.beginPath();ctx.ellipse(fx,fy,fr*1.8,fr*.52,0,0,Math.PI*2);ctx.fill();}ctx.restore();
