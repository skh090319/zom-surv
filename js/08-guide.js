...
function drawMonsterGuide() {
  ...
  const bossW=Math.min(920,canvas.width-48),bossH=280,bossStart=top+normalSectionH+48;
  ...
  bossEntries.forEach((entry,i)=>{const x=bossX,y=bossStart+i*(bossH+gap)-guideScrollY;if(y+bossH<top||y>canvas.height)return;const g=ctx.createLinearGradient(x,y,x+bossW,y+bossH);g.addColorStop(0,`${entry.color}32`);g.addColorStop(.42,`${entry.color}12`);g.addColorStop(1,"rgba(5,7,14,.98)");ctx.save();ctx.shadowColor=entry.color;ctx.shadowBlur=16;drawRoundedRect(x,y,bossW,bossH,22,g,`${entry.color}a0`,2);ctx.restore();
    const artW=Math.min(300,bossW*.39),cx=x+artW/2,cy=y+bossH/2;const aura=ctx.createRadialGradient(cx,cy,18,cx,cy,artW*.48);aura.addColorStop(0,`${entry.color}55`);aura.addColorStop(.55,`${entry.color}16`);aura.addColorStop(1,`${entry.color}00`);ctx.fillStyle=aura;ctx.fillRect(x+5,y+5,artW-10,bossH-10);ctx.save();ctx.translate(cx,cy);ctx.strokeStyle=`${entry.color}40`;ctx.lineWidth=1.5;for(let ring=0;ring<3;ring++){ctx.beginPath();ctx.ellipse(0,62-ring*8,artW*.38-ring*13,20-ring*4,0,0,Math.PI*2);ctx.stroke();}ctx.restore();
    const img=raidBossImages[entry.spriteIndex];if(img?.complete&&img.naturalWidth){const maxW=artW*.78,maxH=bossH*.78,sc=Math.min(maxW/img.naturalWidth,maxH/img.naturalHeight);ctx.save();ctx.shadowColor=entry.color;ctx.shadowBlur=25;ctx.drawImage(img,cx-img.naturalWidth*sc/2,cy-img.naturalHeight*sc/2-7,img.naturalWidth*sc,img.naturalHeight*sc);ctx.restore();}
    ctx.strokeStyle=`${entry.color}45`;ctx.beginPath();ctx.moveTo(x+artW,y+24);ctx.lineTo(x+artW,y+bossH-24);ctx.stroke();const tx=x+artW+25,tw=bossW-artW-48;ctx.textAlign="left";ctx.fillStyle=entry.color;ctx.font="bold 11px Arial";ctx.fillText(entry.tag,tx,y+35);ctx.fillStyle="#fff";ctx.font="900 29px Arial";ctx.fillText(entry.name,tx,y+70);ctx.fillStyle="#c5cede";ctx.font="14px Arial";wrapTextLeft(entry.desc,tx,y+102,tw,22);ctx.strokeStyle=`${entry.color}42`;ctx.beginPath();ctx.moveTo(tx,y+164);ctx.lineTo(x+bossW-24,y+164);ctx.stroke();ctx.fillStyle=entry.color;ctx.font="bold 12px Arial";ctx.fillText("생존 요령",tx,y+190);entry.tips.forEach((tip,n)=>{ctx.fillStyle=entry.color;ctx.fillText("◆",tx+2,y+218+n*27);ctx.fillStyle="#e0e6f0";ctx.font="13px Arial";ctx.fillText(tip,tx+23,y+218+n*27);});
  });ctx.restore();
}
