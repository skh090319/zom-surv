...
drawMareEffects=function(){
  ...
  for(const e of currents){
    ...
    ctx.bezierCurveTo(len*.28,-half*.88,len*.68,-half*.58,len,-half*.08);
    ctx.bezierCurveTo(len*.68,half*.62,len*.3,half*.92,0,half*.34);
    ctx.closePath();
    ctx.fill();
    ...
    ctx.bezierCurveTo(len*.3,-half*.83,len*.7,-half*.5,len,-half*.06);
    ctx.stroke();
    ctx.shadowBlur=0;
    for(let i=0;i<3;i++){
      ...
      ctx.beginPath();
      ctx.moveTo(8,offset);
      ctx.bezierCurveTo(len*.3,offset-half*(.38-i*.09),len*.66,offset+half*(.22-i*.05),len*.95,offset*.25);
      ctx.stroke();
    }
    ctx.restore();
  }
